...
function hideLoading() {
  ...
  // After the overlay's 300ms fade-out, settle the sprite cursor to its normal state
  if (window.refreshCursorState) setTimeout(window.refreshCursorState, 350);
}
...
function renderHomePage() {
  ...
  if (!window.__reducedMotion) {
    ...
    if (heroSection) {
      ...
      const heroTagsEl = heroSection.querySelector('.hero-tags');

      heroSection.style.opacity = '0';
      if (heroAvatar) {
        heroAvatar.style.opacity = '0';
        heroAvatar.style.transform = 'scale(0.8) rotate(-5deg)';
      }
      if (heroContent) {
      ...
      if (heroTagsEl) {
        heroTagsEl.style.opacity = '0';
        heroTagsEl.style.transform = 'translateY(12px)';
      }
      ...
      requestAnimationFrame(() => {
        ...
        setTimeout(() => {
          if (heroContent) {
            heroContent.style.transition = 'opacity 0.5s ease, transform 0.5s cubic-bezier(0.16, 1, 0.3, 1)';
        ...
        setTimeout(() => {
          if (heroTagsEl) {
            heroTagsEl.style.transition = 'opacity 0.5s ease, transform 0.5s cubic-bezier(0.16, 1, 0.3, 1)';
            heroTagsEl.style.opacity = '1';
            heroTagsEl.style.transform = 'translateY(0)';
          }
        }, 300);
      });
    }
...
// Sprite-sheet cursor following the pointer. States: arrow / link / text /
// wait / busy (progress = "working in background"). Theme decides sprite set:
// light → Normal Miku, dark → Snow Miku. Degrades to the static .cur fallback
// if sprites.json fails to load.
function initCustomCursor() {
  ...
  // Native cursor is hidden (cursor:none) while the sprite cursor is active,
  // so computed cursor styles can't drive state — use explicit selectors instead.
  const LINK_SEL = ['a', 'button', '[role="button"]', '[role="link"]', '[onclick]', 'select', 'summary', 'label',
    'input[type="submit"]', 'input[type="button"]', 'input[type="checkbox"]', 'input[type="radio"]',
    '.nav-btn', '.scheme-option', '.category-card', '.novel-card', '.link-card', '.interest-card',
    '.back-to-top', '.theme-toggle', '.admin-toggle', '.color-scheme-toggle', '.settings-btn',
    '.sort-btn', '.reader-nav-btn', '.timeline-item', '.social-link', '.admin-tab', '.admin-action',
    '.select-trigger', '.select-option', '.back-btn', '.admin-small-btn'
  ].join(',');
  const TEXT_SEL = ['input[type="text"]', 'input[type="password"]', 'input[type="url"]', 'input[type="email"]',
    'input[type="search"]', 'input[type="number"]', 'input[type="tel"]', 'input:not([type])',
    'textarea', '[contenteditable="true"]'
  ].join(',');

  const set = () => {
    if (!meta) return { arrow: { hotspot: [0, 0], src: '', duration: 0 } };
    const dark = document.documentElement.dataset.theme === 'dark';
    return (dark ? meta.sets.snowmiku : meta.sets.miku) || meta.sets.miku;
  };

  const waitActive = () => {
    const ov = document.querySelector('.loading-overlay');
    return !!ov && getComputedStyle(ov).visibility === 'visible';
  };

  const detect = (target) => {
    if (!target || target.nodeType !== 1) return 'arrow';
  ...
  const updatePos = () => {
    ...
    const entry = set()[state] || set().arrow;
    el.style.transform = 'translate(' + (mx - entry.hotspot[0] / 2) + 'px,' + (my - entry.hotspot[1] / 2) + 'px)';
  };
  ...
  const applyState = (s) => {
    ...
    Array.prototype.forEach.call(el.classList, (c) => {
      if (c.indexOf('state-') === 0) el.classList.remove(c);
    });
    el.classList.add('state-' + s);
    const entry = set()[s] || set().arrow;
    if (img.getAttribute('src') !== entry.src) img.setAttribute('src', entry.src);
    if (entry.duration > 0) {
      el.style.setProperty('--cur-dur', entry.duration + 'ms');
  ...
  const refreshState = () => {
    let s;
    if (busy) s = 'busy';
    else if (waitActive()) s = 'wait';
    else s = detect(lastTarget);
    applyState(s);
  };

  // In-flight requests (fetch + XHR) show the "working in background" cursor
  let pending = 0;
  let manualBusy = 0;
  const syncBusy = () => {
    busy = manualBusy > 0 || pending > 0;
    refreshState();
  };
  const wrapFetch = () => {
    ...
    window.fetch = function () {
      pending++;
      if (pending === 1) syncBusy();
      return orig.apply(this, arguments).finally(() => {
        pending--;
        if (pending === 0) syncBusy();
      });
    };
  };
  const wrapXHR = () => {
    ...
    const Wrapped = function () {
      ...
      x.send = function () {
        ...
        x.addEventListener('loadend', end);
  ...
  wrapFetch();
  wrapXHR();
  window.setCursorBusy = (b) => { manualBusy += b ? 1 : -1; if (manualBusy < 0) manualBusy = 0; syncBusy(); };
  window.refreshCursorState = refreshState;

  // Re-apply current state when the theme switches (miku <-> snowmiku sprites)
  new MutationObserver(() => { if (meta && visible) applyState(state); })
  ...
  document.addEventListener('dragstart', () => { visible = false; el.classList.remove('on'); });
  document.addEventListener('dragend', () => { visible = true; el.classList.add('on'); updatePos(); });

  fetch('cursors/sprites/sprites.json')
    .then(r => { if (!r.ok) throw new Error('sprites.json ' + r.status); return r.json(); })
    .then((m) => {
      meta = m;
      let failed = false;
      // Preload every sprite; on any failure fall back to the static .cur set
      Object.keys(m.sets).forEach((sn) => {
        Object.keys(m.sets[sn]).forEach((k) => {
          const i = new Image();
          i.onerror = () => {
            if (failed) return;
            failed = true;
            el.remove();
            document.documentElement.classList.remove('cursor-anim-on');
          };
          i.src = m.sets[sn][k].src;
        });
      });
      document.documentElement.classList.add('cursor-anim-on');
      lastTarget = document.elementFromPoint(mx || window.innerWidth / 2, my || window.innerHeight / 2) || document.body;
      applyState(detect(lastTarget));
      visible = true;
      el.classList.add('on');
      updatePos();
      // Loading overlay fades out 300ms after init; settle cursor to arrow then
      setTimeout(() => { if (meta && visible) refreshState(); }, 400);
    })
    .catch(() => el.remove());
}
